...
import config
import emoji
from model_selection_final import ModelID, ModelSelectionAgent
...
class ConvAIRLLBot:
    ...
    def observe(self, m):
        ...
        # Not a new chat:
            ...
            # Control statement for allowed models
            ...
            # End with \model end <model_name>
                ...
                if controls[1] == 'start':
                    ...
                    logging.info("Control msg accepted, selecting model {}".format(controls[2]))
                else:
                    self.ai[chat_id]['allowed_model'] = ModelID.ALL
    ...
    def act(self, chat_id, state,  m):
        ...
        if chat_id not in self.ai:
            ...
            if m['message']['chat']['id'] == chat_id and m['message']['text'] == '/end':
                ...
                data['text'] = '/end'
                data['evaluation'] = {
                    'quality': 5, 'breadth': 5, 'engagement': 5
                }
        ...
        if state != ChatState.CHATTING:
            ...
            elif state == ChatState.START:
                text = "Hello! I hope you're doing well. I am doing fantastic today! " \
            ...
            outgoing_msg_queue.put(
                ({'text': text, 'chat_id': chat_id,
                  'model_name': model_name, 'policyID': policy_id}, {}))
        else:
...
def response_receiver():
    """Receive response from Telegram.
...
def test_app():
    ...
    text = random.choice([
        'ok, how was it', 'wow good observation', 'where did it happen?',
        'can you explain?', 'ok', 'hmm', 'good',
...
if __name__ == '__main__':
    ...
    response_receiver_thread = Thread(target=response_receiver)
